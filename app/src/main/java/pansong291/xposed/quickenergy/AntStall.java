    ...
    private static void sendBack(String billNo, String seatId, String shopId, String shopUserId) {
        ...
        try {
            ...
            if ("SUCCESS".equals(jo.getString("resultCode"))) {
                ...
                jo = new JSONObject(s);
                if ("SUCCESS".equals(jo.getString("resultCode"))) {
                    Log.farm("蚂蚁新村请走[" + FriendIdMap.getNameById(shopUserId) + "]的小摊" + (amount > 0 ? "获得金币" + amount : ""));
            ...
            if ("SUCCESS".equals(jo.getString("resultCode"))) {
                ...
                for (int i = 0; i < friendRankList.length(); i++) {
                    ...
                    if (friend.getBoolean("canInviteOpenShop")) {
                        ...
                        jo = new JSONObject(s);
                        if ("SUCCESS".equals(jo.getString("resultCode"))) {
                            Log.farm("蚂蚁新村邀请[" + FriendIdMap.getNameById(friendUserId) + "]开店成功");
                            return;
                        }
                    }
                }
